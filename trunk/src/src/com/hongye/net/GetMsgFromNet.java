	...
	private static void getAnalyesMsg(){
		...
		try {
			...
			for(int i = 0; i < tmp.length ; i++ ){
				System.out.println(i+". result = "+tmp[i]);
			}
			...
			for(String str : tmp){
				...
				for(int i = 0; i < 6 ; i++){
					strBuild.append(record[i]).append(" ").toString();
				}
				
				String date = strBuild.toString();
				String homeTeamName = record[6];
				String guestTeamName = record[8];
				String gameRst = record[7];
				...
				gameResultDO.setGameRst(gameRst);
				gameResultDO.setDate(DateUtil.convertDate(date));
